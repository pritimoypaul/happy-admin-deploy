...
export function AssignCompanyForm({ id, refetch }: any) {
  ...
  const UPAZILLAS = companys satisfies Framework[];
  ...
  async function onSubmit() {
    ...
    const companyData = {
      companies: trasformed,
    };

    try {
      setLoading(true);
      await axiosInstance.put(`/dealers/${id}/assign-companies`, companyData);

      toast({
        variant: "default",
        title: "Company assigned successfully!",
        description: "You have assigned company to a dealer.",
      });
      setLoading(false);
      refetch();
      setSuccess(true);
  ...
  useEffect(() => {
    if (companyFetched) {
      const transformedData = companyData?.data.result.map((item: any) => ({
        value: item._id,
        label: item.bnName,
      }));
      setCompanys(transformedData);
    }
  }, [companyData]);
