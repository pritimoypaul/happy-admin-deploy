...
export function AssignDealerForm({ id, refetch }: any) {
  ...
  const UPAZILLAS = dealers satisfies Framework[];
  ...
  const { data: dealerData, isFetched: dealerFetched } = useDealerList(1000, 1);
  ...
  async function onSubmit() {
    ...
    try {
      setLoading(true);
      await axiosInstance.put(`/sr/${id}/update-info`, dealerData);

      toast({
        variant: "default",
        title: "Dealer assigned successfully!",
        description: "You have assigned dealer to a sr.",
      });
      setLoading(false);
      refetch();
      setSuccess(true);
  ...
  useEffect(() => {
    if (dealerFetched) {
      const transformedData = dealerData?.data?.result.map((item: any) => ({
        value: item?.dealer?._id,
        label: item?.dealer?.name,
      }));
